		...
		data = {
			commands = {
				...
				police = {
					...
					{cmd = 'tt' , description = 'Запрос место расположение(всех)' , text = '/r Всем!! Доложите свое местоположение в рацию&/r Если вы с напарником то и его жетон', arg = '' , enable = true , waiting = '2.5', bind = "{}"},
					...
					{cmd = 'find' , description = 'Поиск игрока' ,  text = '/find {arg_id}&/me достал{sex} свой КПК и зайдя в базу данных {fraction_tag} открыл{sex} дело гражданина N{arg_id}&/me нажал{sex} на кнопку GPS отслеживания местоположения гражданина' , arg = '{arg_id}' , enable = true, waiting = '2.5', bind = "{}"},
					...
					{cmd = 'gtm' , description = 'Повести за собой' ,  text = '/gotome {arg_id}&/me крепко схватив задержанного, взял{sex} его за руки&/do Задержанный идёт в конвое.', arg = '{arg_id}', enable = true, waiting = '2.5', bind = "{}", in_fastmenu = true},
...
imgui.OnFrame(
    ...
    function(player)
		if player_id ~= nil and isParamSampID(player_id) then
			...
			if imgui.BeginChild('sobes1', imgui.ImVec2(240 * settings.general.custom_dpi, 182 * settings.general.custom_dpi), true) then
				...
				if imgui.Button(fa.USER .. u8" Служебный транспорт", imgui.ImVec2(-1, 25 * settings.general.custom_dpi)) then
					lua_thread.create(function()
						...
						wait(2300)
						sampSendChat("а автомобиль тайота авенс с должности Старшого Инструктора")
						wait(2300)
						sampSendChat("Всем спасибо, лекция окончена, Вы свободны.")
					end)	
				end
				
				if imgui.Button(fa.PLAY .. u8" дресс код", imgui.ImVec2(-1, 25 * settings.general.custom_dpi)) then
					lua_thread.create(function()
					wait(2300)						
				...
				if imgui.Button(fa.CHECK .. u8" Пожарная безопасность", imgui.ImVec2(-1, 25 * settings.general.custom_dpi)) then
					lua_thread.create(function()
						...
						wait(2300)
						sampSendChat("Как мы знаем, за последнее время количество пожаров")
						wait(2300)
						sampSendChat("...сильно увеличилось.")
						wait(2300)
						sampSendChat("Пожар это очень страшная вещь , как и огонь.")
						wait(2300)
						sampSendChat("Чтобы не стать жертвой, нужно соблюдать пару простых правил:")
						wait(2300)
						sampSendChat("Не оставляйте без присмотра включенные электроприборы,")
						wait(2300)
						sampSendChat("...особенно утюги, обогреватели.")
						wait(2300)
						sampSendChat("Уходя из дома, не забудь выключить все приборы,")
						wait(2300)
						sampSendChat("...дабы избежать возгорания.")
						wait(2300)
						sampSendChat("Если же вы почувствовали запах газа, то не зажигайте")
						wait(2300)
						sampSendChat("...спички и не включайте свет.")
						wait(2300)
						sampSendChat("Всё это может привести к возгоранию или же пожару.")
						wait(2300)
						sampSendChat("Благодарю за прослушивание моей лекции, на этом все.")
						wait(2300)
						sampSendChat("Все за работу, всем удачного дня.")
				...
				if imgui.Button(fa.USER_PLUS .. u8" ПДД", imgui.ImVec2(-1, 25 * settings.general.custom_dpi)) then
					lua_thread.create(function()
						...
						wait(2300)
						sampSendChat("Все мы знаем, что ПДД расшифровывается, как")
						wait(2300)
						sampSendChat("...Правила Дорожного Движения.")
						wait(2300)
						sampSendChat("В данной лекции я ещё раз расскажу вам обо всём.")
						wait(2300)
						sampSendChat("По поводу ограничений скорости. В городе максимальная")
						wait(2300)
						sampSendChat("...60 км/ч, вне города - 120 км/ч.")
						wait(2300)
						sampSendChat("Остановка и стоянка транспортного средства")
						wait(2300)
						sampSendChat("...разрешена на обочине или же в отведенных для")
						wait(2300)
						sampSendChat("...этого местах.")
						wait(2300)
						sampSendChat("В экстренных случаях можно остановится на тротуаре.")
						wait(2300)
						sampSendChat("При понижении видимости, либо же тумане - снизить")
						wait(2300)
						sampSendChat("...скорость и включить фары.")
						wait(2300)
						sampSendChat("Дорогу переходить можно только на зелёный свет.")
						wait(2300)
						sampSendChat("Благодарю за прослушивание моей лекции, на этом все.")
						wait(2300)
						sampSendChat("Все за работу, всем удачного дня.")
			...
			imgui.SameLine()
			if imgui.BeginChild('sobes2', imgui.ImVec2(240 * settings.general.custom_dpi, 182 * settings.general.custom_dpi), true) then
				imgui.CenterColumnText(fa.BOOKMARK..u8" Недоступно " .. fa.BOOKMARK)
				imgui.Separator()
				if imgui.Button(fa.GLOBE .. u8" Недоступно ", imgui.ImVec2(-1, 25 * settings.general.custom_dpi)) then
					sampSendChat("")
				end
				if imgui.Button(fa.CIRCLE_QUESTION .. u8" Недоступно ", imgui.ImVec2(-1, 25 * settings.general.custom_dpi)) then
					sampSendChat("")
				end
				if imgui.Button(fa.CIRCLE_QUESTION .. u8" Недоступно ", imgui.ImVec2(-1, 25 * settings.general.custom_dpi)) then
					sampSendChat("")
