...
function load_settings()
    ...
    -- Force dark theme if custom (MoonMonet) was selected - not supported on PC
    if settings.general and settings.general.helper_theme == 0 then
...
local modules = {
	commands = {
		...
		data = {
			...
			commands_manage = {
				my = {},
				goss = {
					{cmd = 'inv' , description = 'Принятие игрока в организацию' , text = '/do В кармане есть связка с ключами от раздевалки.&/me достаёт из кармана один ключ из связки ключей от раздевалки&/todo Возьмите, это ключ от нашей раздевалки*передавая ключ человеку напротив&/invite {arg_id}&/r По приветствуем нового сотрудника {get_rp_nick({arg_id})}' , arg = '{arg_id}', enable = true, waiting = '2.5'  , bind = "{}", in_fastmenu = true  },
					...
					{cmd = 'vig' , description = 'Выдача выговора cотруднику' , text = '/do КПК в руке.&/do База данных {fraction_tag} открыта.&/me Изменил информацию о сотруднике.&/fwarn {arg_id} {arg2}&/r Сотруднику {fraction_tag} {get_rp_nick({arg_id})} был выдан выговор за:{arg2}' , arg = '{arg_id} {arg2}', enable = true, waiting = '2.5'  , bind = "{}"},
					{cmd = 'unvig' , description = 'Снятие выговора cотруднику' , text = '/do КПК в руке.&/do База данных {fraction_tag} открыта.&/me Изменил информацию о сотруднике.&/fwarn {arg_id} {arg2}&/r Сотруднику {fraction_tag} {get_rp_nick({arg_id})} был снят выговор!' , arg = '{arg_id}', enable = true, waiting = '2.5', bind = "{}" , in_fastmenu = true  },
					{cmd = 'unv' , description = 'Увольнение игрока из фракции' , text = '/do КПК в руке.&/me В базе данных {fraction_tag} была убрана информация о сотруднике.&/uninvite {arg_id} {arg2}&/r Сотрудник {get_rp_nick({arg_id})} был уволен, причина:{arg2}' , arg = '{arg_id} {arg2}', enable = true, waiting = '2.5', bind = "{}"   },
				...
				police_gov = {
					...
					{cmd = 'point' , description = 'Устаовить метку для сотрудников' , text = '/r Срочно выдвигайтесь ко мне, отправляю вам координаты...&/point' , arg = '', enable = true, waiting = '2.5', bind = "{}"},
					{cmd = 'govka' , description = 'Собеседование по госс.волне' , text = '/d [{fraction_tag}] - [Всем] Занимаю гос. волну.&/gov [{fraction_tag}] На данный момент пройдет собеседование в {fraction}&/gov [{fraction_tag}] При себе иметь: 3 летнюю прописку, мед.карту, лицензии и военный билет&/gov [{fraction_tag}] Всех желающих ждем в Холле Полиции Арзамаса.&/d [{fraction_tag}] - [Всем] Освобождаю гос. волну.' , arg = '', enable = true, waiting = '2.5', bind = "{}"},
				},			
				hospital_gov = {
					{cmd = 'inv' , description = 'Принятие игрока в организацию' , text = '/do В кармане есть связка с ключами от раздевалки.&/me достаёт из кармана один ключ из связки ключей от раздевалки&/todo Возьмите, это ключ от нашей раздевалки*передавая ключ человеку напротив&/invite {arg_id}&/n @{get_nick({arg_id})} , примите предложение чтобы получить инвайт!' , arg = '{arg_id}', enable = true, waiting = '2.5'  , bind = "{}", in_fastmenu = true  },
					{cmd = 'gr' , description = 'Повышение/понижение cотрудника' , text = '{show_rank_menu}&/me достаёт из кармана свой телефон и заходит в базу данных {fraction_tag}&/me изменяет информацию о сотруднике {get_ru_nick({arg_id})} в базе данных {fraction_tag}&/me выходит с базы данных и убирает телефон обратно в карман&/giverank {arg_id} {get_rank}&/r Сотрудник {get_ru_nick({arg_id})} получил новую должность!' , arg = '{arg_id}', enable = true, waiting = '2.5', bind = "{}", in_fastmenu = true   },	
					...
					{cmd = 'unv' , description = 'Увольнение игрока из фракции' , text = '/me достаёт из кармана свой телефон и заходит в базу данных {fraction_tag}&/me изменяет информацию о сотруднике {get_ru_nick({arg_id})} в базе данных {fraction_tag}&/me выходит с базы данных и убирает свой телефон обратно в карман&/uninvite {arg_id} {arg2}&/r Сотрудник {get_ru_nick({arg_id})} был уволен по причине: {arg2}' , arg = '{arg_id} {arg2}', enable = true, waiting = '2.5', bind = "{}"   },
					{cmd = 'point' , description = 'Устаовить метку для сотрудников' , text = '/r Срочно выдвигайтесь ко мне, отправляю вам координаты...&/point' , arg = '', enable = true, waiting = '2.5', bind = "{}"},
					{cmd = 'govka' , description = 'Собеседование по госс.волне' , text = '/d [{fraction_tag}] - [Все] Занимаю Государственную волну вещания.&/gov [{fraction_tag}] Уважаемые жители Восточного Округа, минуточку внимания.&/gov [{fraction_tag}] Сообщаю вам, что проходит собеседование в {fraction_tag}!&/gov [{fraction_tag}] Просьба прибыть в Холл Больницы.&/d [{fraction_tag}] - [Все] Освобождаю Государственную волну вещания.' , arg = '', enable = true, waiting = '2.5', bind = "{}"},
				},		
				smi_gov = {
					{cmd = 'inv' , description = 'Принятие игрока в организацию' , text = '/do В кармане есть связка с ключами от раздевалки.&/me достаёт из кармана один ключ из связки ключей от раздевалки&/todo Возьмите, это ключ от нашей раздевалки*передавая ключ человеку напротив&/invite {arg_id}&/n @{get_nick({arg_id})} , примите предложение чтобы получить инвайт!' , arg = '{arg_id}', enable = true, waiting = '2.5'  , bind = "{}", in_fastmenu = true  },
					{cmd = 'gr' , description = 'Повышение/понижение cотрудника' , text = '{show_rank_menu}&/me достаёт из кармана свой телефон и заходит в базу данных {fraction_tag}&/me изменяет информацию о сотруднике {get_ru_nick({arg_id})} в базе данных {fraction_tag}&/me выходит с базы данных и убирает телефон обратно в карман&/giverank {arg_id} {get_rank}&/r Сотрудник {get_ru_nick({arg_id})} получил новую должность!' , arg = '{arg_id}', enable = true, waiting = '2.5', bind = "{}", in_fastmenu = true   },	
					...
					{cmd = 'unv' , description = 'Увольнение игрока из фракции' , text = '/me достаёт из кармана свой телефон и заходит в базу данных {fraction_tag}&/me изменяет информацию о сотруднике {get_ru_nick({arg_id})} в базе данных {fraction_tag}&/me выходит с базы данных и убирает свой телефон обратно в карман&/uninvite {arg_id} {arg2}&/r Сотрудник {get_ru_nick({arg_id})} был уволен по причине: {arg2}' , arg = '{arg_id} {arg2}', enable = true, waiting = '2.5', bind = "{}"   },
					{cmd = 'point' , description = 'Устаовить метку для сотрудников' , text = '/r Срочно выдвигайтесь ко мне, отправляю вам координаты...&/point' , arg = '', enable = true, waiting = '2.5', bind = "{}"},
					{cmd = 'govka' , description = 'Собеседование по госс.волне' , text = '/d [ГТРК] - [Всем] Занимаю Государственную волну вещания.&/gov [ГТРК] Уважаемые жители Восточного Округа, минуточку внимания.&/gov [ГТРК] Сообщаю вам, что проходит собеседование в Радиостанцию г.Арзамас!&/gov [ГТРК] Ждем всех!&/d [ГТРК] - [Всем] Освобождаю Государственную волну вещания.' , arg = '', enable = true, waiting = '2.5', bind = "{}"},
				},					
				goss_fcb = {
				...
				goss_gov = {
					...
					{cmd = 'unv' , description = 'Увольнение игрока из фракции' , text = '/me достаёт из кармана свой телефон и заходит в базу данных {fraction_tag}&/me изменяет информацию о сотруднике {get_ru_nick({arg_id})} в базе данных {fraction_tag}&/me выходит с базы данных и убирает свой телефон обратно в карман&/uninvite {arg_id} {arg2}&/r Сотрудник {get_ru_nick({arg_id})} был уволен по причине: {arg2}' , arg = '{arg_id} {arg2}', enable = true, waiting = '2.5', bind = "{}"   },
					{cmd = 'point' , description = 'Устаовить метку для сотрудников' , text = '/r Срочно выдвигайтесь ко мне, отправляю вам координаты...&/point' , arg = '', enable = true, waiting = '2.5', bind = "{}"},
					{cmd = 'govka' , description = 'Собеседование по госс.волне' , text = '' , arg = '', enable = true, waiting = '2.5', bind = "{}"},
				},	
				mafia = {
	...
	notes = {
		...
		data = {
			{ note_name = 'Глава 1 (ФП)', note_text = "Глава I. Общее положение&&1.1. Запрещено нарушать Законы Округа (Уголовный кодекс и прочие грубые нарушения, не входящие в компетентность сотрудника) - Понижение | Выговор | Увольнение&1.2. Запрещено заниматься деятельностью, не связанной с исполнением служебных обязанностей, в течение рабочего дня - Предупреждение | Выговор | Увольнение&1.3. Запрещено использовать личный транспорт в рабочих целях находясь при исполнение - Предупреждение | Выговор&Исключение: Разрешено только если транспорт находится в реестре.&1.4. Запрещено бездействовать при Чрезвычайных Ситуациях и Военном положение - Выговор | Увольнение&1.5. Запрещено бездействие или неисполнение обязанностей сотрудниками входящих в компетенции по оказанию помощи лицам, находящимся в опасной для жизни ситуации - Выговор | Понижение&1.6. Запрещено употребление алкоголя в рабочее время – Выговор | Понижение | Увольнение&1.7. Запрещено намеренно не реагировать на сообщения, поступающие по рации департамента, в рабочее время - Выговор | Понижение&Примечание: В не рабочее время, данная статья не вменяется.&1.8. Запрещено сотрудникам других организаций участвовать в спецоперациях, проводимых ФСБ, без прямого указания ФСБ - Предупреждение | Выговор&1.9. Запрещено нарушать субординацию при общении c гражданскими, и выше по должностям людьми - Выговор | Понижение&1.10. Запрещено предоставлять заведомо ложную информацию сотрудникам государственных органов или вышестоящим должностным лицам - Выговор | Понижение | Увольнение.&1.11. Запрещено нарушать устав и прочие регламенты организации - Выговор | Понижение | Увольнение&1.12. Запрещено применять физическую силу в сторону сотрудников ФСБ - Выговор | Увольнение&Примечание: За исключением статьи 2.3 ФП&"},		
			{ note_name = 'Глава 2 (ФП)', note_text = "2.1. Во главе всех государственных организаций стоит Губернатор, Вице-губернатор, Директор ФСБ, Зам Директора ФСБ&2.2. Руководство, а также сотрудники Федеральной Службы Безопасности напрямую подчиняются Президиуму Восточного Округа. Выговор | Понижение&2.3. Сотрудников Правительства и ФСБ допустимо задерживать при наличии особых оснований - Выговор | Понижение | Увольнение.&Примечание: Особым основаниям является гос. измена или же прямое нанесение вреда здоровью и жизни окружающих.&2.4. Запрещено проникать на территорию Федеральной Службы Безопасности без разрешения Руководства ФСБ - Предупреждение | Выговор | Понижение&2.5. Запрещено не появляться на построения, контролируемые Руководством (8+) Правительство/ФСБ - Выговор | Понижение&2.6. Запрещено вести слежку за руководством Правительства и ФСБ (8+) - Выговор | Понижение | Увольнение&Примечание: С разрешения Президиума (ГС ГОС)&2.7. Сотрудники ФСБ могут снять неприкосновенность с любого лица находящегося в Федеральном Розыске.&2.8. Губернатор имеет право проходить на охраняемы объекты Округа с уведомлением руководства.&Примечание: В некоторых случаях он может получить обоснованный отказ.&2.9 Запрещено нарушать закон имея неприкосновенность. Понижение | Увольнение"},	
			{ note_name = 'Глава I. (ПДД)', note_text = "1.1. Езда по встречной. Штраф в размере 50.000 рублей.&1.2. Езда по встречной на грузовом авто. Штраф в размере 100.000 рублей&"},		
			{ note_name = 'Глава II. (ПДД)', note_text = "2.1. Наезд на пешехода. Присуждается 3 уровень розыска и изъятие прав.&Если смертельной травмы не произошло, вместо розыска накладывается штраф 200.000 рублей, который подлежит выплате пострадавшему."},			
			{ note_name = 'Глава III. (ПДД)', note_text = "3.1. Парковка в неположенном месте легковым транспортным средством. Штраф в размере 45.000 рублей / эвакуация транспорта на штраф стоянку.&3.2. Парковка в неположенном месте грузовым авто. Штраф в размере 75.000 рублей. Изъятие вод.прав.&3.3. Парковка в неположенном месте воздушным транспортным средством. Штраф в размере 150.000 рублей / изъятие лицензии на управление воздушным Т/С."},		
			{ note_name = 'Глава IV. (ПДД)', note_text = "4.1. Уход с места ДТП. Присуждается 2 уровень розыска, штраф: 60.000 рублей / изъятие прав.&4.2. Виновник ДТП. Штраф в размере 50.000 рублей, выплата материального ущерба.&4.2. Таран транспортного средства. Присуждается 2 уровень розыска / штраф 250.000 руб."},
			{ note_name = 'Глава V. (ПДД)', note_text = "5.1. Движение по обочинам, тротуарам, ж/д путям, трамвайным рельсам и полям. Штраф в размере 50.000 рублей / изъятие прав.&5.2. За переход проезжей части пешеходом в неположенном месте. Штраф в размере 45.000 руб."},			
			{ note_name = 'Глава VI. (ПДД)', note_text = "6.1. За игнорирование спец. сирен. Штраф в размере 100.000 рублей, изъятие прав.&6.2. За игнорирование спец. сирен, которые включены чтобы остановить вас. Присуждается 3 уровня розыска, изъятие прав."},			
			{ note_name = 'Глава VII. (ПДД)', note_text = "7.1. За не включенные поворотники при совершении маневра. Штраф в размере 30.000 рублей.&7.2. За стоянку посреди дороги без аварийных сигналов / создание аварийной ситуации. Штраф в размере 50.000 рублей.&7.3. За выключенные фары в ночное время суток: Штраф в размере 50.000 рублей&7.4. Езда с аварийными сигналами. Штраф в размере 35.000 рублей"},		
			{ note_name = 'Глава VIII. (ПДД)', note_text = "8.1. Превышение скорости в городе больше 60 км/ч. Штраф в размере 50.000 рублей.&8.2. Превышение скорости вне города больше 120 км/ч. Штраф в размере 100.000 рублей."},			
			{ note_name = 'Глава IX (ПДД)', note_text = "9.1. За движение на неисправном транспортном средстве (Дым из под капота / сильные повреждения у автомобиля: Штраф в размере 95.000 рублей.&9.2. За движение на транспортном средстве с неисправной или сломанной фарой: Штраф в размере 25.000 рублей.&9.3. За отсутствие номерных знаков на транспортном средстве: Машина изымается на штраф.стоянку и штраф в размере 50.000 рублей."},
			{ note_name = 'Глава X. (ПДД)', note_text = "10.1. За проезд на желтый сигнал светофора. Штраф в размере 30.000 рублей&10.2. За проезд на красный сигнал светофора. Накладывается системный штраф, за него строго запрещено&накладывать штраф."},
			{ note_name = 'Информация скрипта', note_text = "Приветствую, уважаемые пользователи Rodina Helper!&Если вы видите данное сообщение — значит, у меня всё получилось: скрипт официально одобрен самим Данилом Лиманским (ГА) на Восточном сервере.&Если всё пойдёт дальше, скрипт появится и на других округах нашего прекрасного проекта. Ожидайте дальнейших обновлений!&Мы стараемся сделать всё для комфортного использования скрипта и вашей защиты. Тот же самый «Полис Хелпер» уступает место данному скрипту.&Далее будет создан сайт и установщик для этого хелпера. Всю информацию вы можете увидеть в Telegram-канале (в настройках скрипта). Там же можно задать вопросы по скрипту и сообщить о проблемах.&Я также являюсь техническим модератором Баркина. Если скрипт станет официальным, он будет выпускаться в сборках у Баркина.&Спасибо, что вы со мной! Удачного использования скрипта!"},
		}
	},
	rpgun = {
		name = 'RP оружие',
...
{get_price_head} - Цена лечения от наркозависимости
		...
		if text:find("Организация:") then
			...
			if settings.player_info.fraction == 'Не имеется' then
				...
				local fraction_data = {
					...
					['Городская полиция'] = {'ГИБДД', 'police'},
					['Полиция округа'] = {'ГУВД', 'police'},
					['Больница округа'] = {'ОКБ', 'hospital'},
					['Городская больница'] = {'ГКБ', 'hospital'},
					['Центр Лицензирования'] = {'МРЭО', 'lc'},
					['Правительство'] = {'Пра-во', 'gov'},
					['Радиоцентр'] = {'ГТРК', 'smi'},
				}
...
imgui.OnInitialize(function()
	...
	if settings.general.helper_theme == 2 then
		apply_white_theme()
	else
		apply_dark_theme()
	end
...
imgui.OnFrame(
    ...
    function(player)
		...
		elseif MODULE.Initial.step == 1 then
			...
			render_org_block(1, fa.BUILDING_SHIELD, 'МВД', 'ГИБДД/ГУВД/ФСБ')
			imgui.SameLine()
			render_org_block(2, fa.HOSPITAL, 'Здраво.', 'ГКБ/ОКБ')
			imgui.SameLine()
		...
		elseif MODULE.Initial.step == 2 then
			...
			local orgs = {
				[1] = {
					{name = "Полиция округа",			    tag = "ГУВД"},
					{name = "Городская полиция", 			tag = "ГИБДД"},
					{name = "Фед.Служба Безопасности", 		tag = "ФСБ"},
				},
				[2] = {
					{name = "Окружная больница",   			tag = "ОКБ"},
					{name = "Городская больница", 			tag = "ГКБ"},
				},
				[3] = {
					{name = "Тюрма Строго Режим", 			tag = "ФСИН"},
					{name = "Армия", 						tag = "Армия"},
				},
				[4] = {
					{name = "СМИ", 							tag = "ГТРК"},
				},
		...
		elseif MODULE.Initial.step == 5 then
			local fraction_modes = {
				...
				{id = 11, name = "Гл. упр. внутренних дел",    mode = "police", 	tag = "ГУВД"},
				{id = 12, name = "Гос. Инспек. Безопасн. Дорож. Движ.",         mode = "police", 	tag = "ГИБДД"},
				{id = 13, name = "Фед. Служба Безопасности",   mode = "fcb", 		tag = "ФСБ"},
				{id = 21, name = "Окружная больница",      	   mode = "hospital", 	tag = "ОКБ"},
				{id = 22, name = "Городская больница",     	   mode = "hospital", 	tag = "ГКБ"},
				{id = 31, name = "Тюрма строго режима",        mode = "prison", 	tag = "ФСИН"},
				{id = 32, name = "Армия",          			   mode = "army", 		tag = "Армия"},
				{id = 41, name = "СМИ",            		       mode = "smi",	 	tag = "ГТРК"},
				{id = 51, name = "Правительство",              mode = "gov", 		tag = "Пра-во"},
				{id = 52, name = "Центр лицензирования",       mode = "lc", 		tag = "МРЭО"},
				{id = 71, name = "Украинская Мафия",           mode = "mafia",		tag = "УМ"},
				{id = 72, name = "Кавказская Мафия",           mode = "mafia", 		tag = "КМ"},
				{id = 73, name = "Русская мафия",              mode = "mafia", 		tag = "РМ"},
			}	
			...
			if imgui.BeginTabItem(fa.HOUSE..u8' Главное меню') then
				...
				if imgui.BeginChild('##2', imgui.ImVec2(589 * settings.general.custom_dpi, 169 * settings.general.custom_dpi), true) then
					...
					if imgui.BeginPopupModal(getHelperIcon() .. u8' Должность в организации ' .. getHelperIcon() .. '##fraction_rank', _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar) then
						...
						imgui.SameLine()
						if imgui.Button(fa.FLOPPY_DISK .. u8' Сохранить ' .. fa.FLOPPY_DISK .. '##save_fraction_rank', imgui.ImVec2(200 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
    ...
    imgui.Columns(1)
					...
					if imgui.BeginTabItem(fa.BARS..u8' Стандартные команды') then 
						if imgui.BeginChild('##99', imgui.ImVec2(589 * settings.general.custom_dpi, 338 * settings.general.custom_dpi), true) then
							...
							if not isMode('none') then
								...
								imgui.NextColumn()
								imgui.CenterColumnText(u8"Контролер АФК")
								imgui.Columns(1)
								imgui.Separator()
							end							
			...
			if imgui.BeginTabItem(fa.GEAR..u8' Настройки') then 
				...
				if imgui.BeginChild("##2_2",imgui.ImVec2(589 * settings.general.custom_dpi, 53 * settings.general.custom_dpi),true, imgui.WindowFlags.NoScrollbar) then
					...
					if false then
						-- removed custom/moonmonet color block
					else
				...
				if imgui.BeginChild("##4",imgui.ImVec2(589 * settings.general.custom_dpi, 35 * settings.general.custom_dpi),true) then
					...
					if imgui.BeginPopupModal(fa.TRIANGLE_EXCLAMATION .. u8' Предупреждение ' .. fa.TRIANGLE_EXCLAMATION .. '##reset', _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar) then
						change_dpi()
						imgui.CenterText(u8'Вы действительно хотите сбросить все данныые хелпера?')
						imgui.Separator()
...
function render_fractions_functions()
	...
	if isMode('smi') then fraction = 'ГТРК' end
	...
	if imgui.BeginTabItem(fa.GEARS .. u8' Функции ' .. u8(fraction)) then
			...
			if imgui.BeginTabBar('FractinFunctions') then
				if imgui.BeginTabItem(fa.ROBOT .. u8' Личный помочник "Ассистент"') then 
					if imgui.BeginChild('##mj_assist', imgui.ImVec2(589 * settings.general.custom_dpi, 338 * settings.general.custom_dpi), true) then
						...
						render_assist_item(
							"Авто-доклад CODE 0",
							"При получении урона автоматически отправляет доклад CODE 0 с указанием ника нападавшего.",
							settings.mj,
							"auto_doklad_damage"
						)
						imgui.Separator()
			...
			if imgui.BeginTabBar('FractinFunctions') then
				if imgui.BeginTabItem(fa.ROBOT .. u8' Личный помочник "Ассистент"') then 
					if imgui.BeginChild('##hospital_assist', imgui.ImVec2(589 * settings.general.custom_dpi, 338 * settings.general.custom_dpi), true) then
						...
						render_assist_item(
							"Хил из чата",
							"Позволяет быстро лечить пациентов которые просят хил.\nЕсть два режима работы хила из чата:\n1) По нажатию кнопки\n2) Автоматический\nДля смены режима используйте кнопочку шестерёнки справа\n\nПо дефолту работает режим по нажатию кнопки.",
				...
				if imgui.BeginTabItem(fa.SACK_DOLLAR .. u8' Ценовая политика больницы') then 
					if imgui.BeginChild('##hospital_price', imgui.ImVec2(589 * settings.general.custom_dpi, 338 * settings.general.custom_dpi), true) then
						local med_price_fields = {
							{label = '  Не подключено, все цены в биндах',              		      key = 'heal'},							
						}
		...
		elseif isMode('gov') then
			...
			if imgui.BeginChild('##assist', imgui.ImVec2(589 * settings.general.custom_dpi, 367 * settings.general.custom_dpi), true) then
				firs_render_assist_gui()
				imgui.Separator()
				imgui.CenterText(u8('Ваша фракция временно не поддерживаеться!'))
							...
							for _, item in ipairs(chapter.item) do
								if item.text and item.text:rupper():find(input_sum_decoded:rupper()) or input_sum_decoded == '' then
									...
									if imgui.BeginPopupModal(popup_id, nil, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize) then
										...
										imgui.SameLine()
										local text_rank = ((settings.general.fraction == 'ФСБ' or settings.general.fraction == 'fcb') and ' [4+]' or ' [5+]')
